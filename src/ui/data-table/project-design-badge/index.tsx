      ...
      <Tooltip.Provider>
        <Tooltip.Root>
          <Tooltip.Trigger asChild>
            <div 
              className={`flex items-center justify-center px-[${statusItem?.paddingX}] py-2 rounded ${statusItem?.colorBg} ${statusItem?.colorBgDark != null ?`dark:${statusItem?.colorBgDark}` : ''} ${statusItem?.colorText} ${statusItem?.colorTextDark != null ?`dark:${statusItem?.colorTextDark}` : ''} text-xs overflow-hidden whitespace-nowrap truncate`}
            >
              {statusItem?.label}
            </div>
          </Tooltip.Trigger>
